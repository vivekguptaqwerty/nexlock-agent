...
class HeartbeatForegroundService : Service() {
    ...
    // IMPORTANCE_MIN (not LOW) deliberately — this is the least visible level Android allows
    // for a foreground service's required notification: no status bar icon, no sound, and
    // collapsed under "Show more" in the shade unless the customer expands it themselves. A
    // foreground service can't ship with zero notification (OS requirement), but this is as
    // close to invisible as the platform permits while keeping the reliability this service
    // exists for.
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            ...
            description = "Background sync for remote lock/unlock."
            setShowBadge(false)
        }
